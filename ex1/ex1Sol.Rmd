...
```{r, include=FALSE}
knitr::opts_chunk$set(fig.align = 'center', out.width = "10cm")
```
...
>> a) Balancing property is not satisfied because x in block 8 is not balanced.  
...
>>> The density function of propensity score is shown in Figure 1.  

```{r fig4,fig.cap='Density Function of Propensity Score', echo=F}
knitr::include_graphics('comsup.png')
```

>>> When the propensity score is close to zero, we have not-small number of observations to be matched in the treated group, but control group only have few observations to be matched when propensity score is close to 1. This lack of matching observations possibly deteriorates the estimation of ATNT when the extreme exists
...
>>> For all estimations, we can say that the estimation is not close to the true values. This stems from the wrong specification of the model, i.e. redundant matching variables of theta square and x\*theta.
